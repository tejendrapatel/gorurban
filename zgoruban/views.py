...
def CONTACT(request):
    ...
    year= Year_categoryss.objects.all()
    zcmap = mcm[:3]
    d = {"zcmap":zcmap,"year":year}
    return render(request, 'frontend/contact.html',d)
######new######
def OPPURTUNITIES(request):
    ...
    zcmap = mcm[:3]
    d = {"zcmap": zcmap,"year":year}
    return render(request, 'frontend/opportunities.html',d)
...
def BLOGSS(request):
    ...
    zcmap = mcm[:3]
    year= Year_categoryss.objects.all()
    d = {"blo":blo,"zcmap": zcmap,"year":year}
...
def BLOG_SINGLE(request,blo_id):
    ...
    blo = BLOGS.objects.all().order_by('-id')
    recent=blo[:3]
    year= Year_categoryss.objects.all()
    d = {"blosingle":blosingle,"recent":recent,"year":year}
    return render(request, 'frontend/blog_single.html',d)
#####NEW##
def GALLERY_SINGLE(request,cmp_id):
    blosingle = Year_categoryss.objects.get(id=cmp_id)
    blo = GALLERYSS.objects.filter(years=blosingle)
    year= Year_categoryss.objects.all()
    d = {"blosingle":blosingle,"blo":blo,"year":year}
    return render(request, 'frontend/gallery.html',d)
#####NEW##
def NEWS_SINGLE(request,cmp_id):
    blosingle = Year_categoryss.objects.get(id=cmp_id)
    blo = NEWSS.objects.filter(years=blosingle)
    year= Year_categoryss.objects.all()
    d = {"blosingle":blosingle,"blo":blo,"year":year}
    return render(request, 'frontend/gallery.html',d)
#####NEW##
def EVENTS_SINGLE(request,cmp_id):
    blosingle = Year_categoryss.objects.get(id=cmp_id)
    blo = EVENTS.objects.filter(years=blosingle)
    year= Year_categoryss.objects.all()
    d = {"blosingle":blosingle,"blo":blo,"year":year}
    return render(request, 'frontend/gallery.html',d)
